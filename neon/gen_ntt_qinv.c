...
// Compiler flags:
// gcc reduce.c gen_ntt_qinv.c -o gen_ntt_qinv -Werror -Wall -Wpedantic -O3 
...
int main()
{
  int16_t neon_zetas_qinv[224] = {0}, neon_zetas_inv_qinv[272] = {0};

  for (int i = 0; i < 224; i++)
  {
    neon_zetas_qinv[i] = (neon_zetas[i] * QINV);
  }
  for (int i = 0; i < 272; i++)
  {
    neon_zetas_inv_qinv[i] = (neon_zetas_inv[i] * QINV);
  }

  print_array(neon_zetas_qinv, 224, "const int16_t neon_zetas_qinv");
  print_array(neon_zetas_inv_qinv, 272, "const int16_t neon_zetas_inv_qinv");
